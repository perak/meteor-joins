...
if(Meteor.isServer) {
	...
	Meteor.startup(function() {
		...
		Tracker.autorun(function() {
			var reactiveJoins = _ReactiveJoins.find();

			function rerunSubscription(name) {
				for(var key in Meteor.default_connection._subscriptions) {
					var tmpSub = Meteor.default_connection._subscriptions[key];
					if(tmpSub.name == name && !tmpSub.inactive && tmpSub.ready) {
						var args = [];
						args.push(name);
						if(tmpSub.params) {
							tmpSub.params.map(function(param) {
								args.push(param);
							});
						}
			...
			reactiveJoins.observe({
				added: function(newDocument) {
					rerunSubscription(newDocument.name);
				},
...
Mongo.Collection.prototype.publishJoinedCursors = function(cursor, options, publication) {
	...
	if(options && options.reactive && publication) {
		cursor.observe({
			...
			changed: function(newDocument, oldDocument) {
// todo: if value of foreign key is changed then rerun subscription
			}
		});
	}
